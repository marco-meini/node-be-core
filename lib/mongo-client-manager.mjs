  ...
  async connect() {
    try {
      ...
      this.client.on("close", () => {
        console.error(`THE CONNECTION TO MONGODB WAS UNEXPECTEDLY CLOSED: event "close"`);
      });
      this.client.on("connectionClosed", (event) => {
        console.error(`THE CONNECTION TO MONGODB WAS UNEXPECTEDLY CLOSED: event connectionClosed, reason ${event.reason}`);
      });
