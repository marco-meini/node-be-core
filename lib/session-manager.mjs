...
class SessionManager {
  ...
  async storeNewSession(sessionData) {
    try {
      let secret = v1();
      let jwtToken = await this.__signToken(sessionData, secret);
  ...
  async updateDeviceSession(refresh_token) {
    try {
      ...
      if (deviceSession && deviceSession._id) {
        let accessToken = await this.__signToken({ id_user: deviceSession.id_user, grants: deviceSession.grants }, deviceSession.refresh_token);
  ...
  async updateDeviceSessionAndGrants(deviceSession) {
    try {
      if (deviceSession && deviceSession._id) {
        deviceSession.access_token = await this.__signToken({ id_user: deviceSession.id_user, grants: deviceSession.grants }, deviceSession.refresh_token);
