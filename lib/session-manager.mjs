...
class SessionManager {
  ...
  /**
   *
   * @param {{ long: number, short: number }} sessionExpiration
   ...
   * @private
   */
  __signToken(payload, secret, persistent = false) {
    return new Promise((resolve, reject) => {
      let expiresIn = (persistent ? this.__sessionExpiration.long : this.__sessionExpiration.short) || 86400;
   ...
   */
  async storeNewSession(sessionData, persistent = false) {
    try {
      let secret = v1();
      let jwtToken = await this.__signToken({ id_user: sessionData.id_user }, secret, persistent);
