...
class PgClientManager {
   ...
   * @returns
   */
  async insert(item, tableName, transactionClient = null) {
    try {
      let columns = [];
      let values = [];
      let indexes = [];
      ...
      /** @type {QueryOptions} */
      let qo = { sql: sql, replacements: values };
      if (transactionClient) qo.transactionClient = transactionClient;
      let insertedItem = await this.queryReturnFirst(qo);
   ...
   * @returns {Promise<void>}
   */
  async updateByKey(item, fieldsToUpdate, keys, tableName, transactionClient = null) {
    try {
      let sets = [];
      let where = [];
      let values = [];
